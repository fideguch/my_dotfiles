...
├── hooks/
│   └── hooks.json         # PreToolUse/PostToolUse/Stop/Session hooks 定義
├── scripts/hooks/         # hooks.json から呼び出される実行スクリプト群
...
`hooks/hooks.json` で定義。全セッションで自動実行される。
...
ln -sf ~/my_dotfiles/claude/hooks/hooks.json ~/.claude/hooks/hooks.json
